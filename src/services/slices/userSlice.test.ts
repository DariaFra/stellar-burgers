...
import { getUserApi, loginUserApi, registerUserApi, updateUserApi } from '../../utils/burger-api';
...
describe('userSlice', () => {
  ...
  it('должен обрабатывать getLogin.rejected', () => {
    ...
    const action = {type: getLogin.rejected.type, error: {message: errorMessage}};
    const state = reducer(initialState, action);
  ...
  it('должен обрабатывать getUpdateUser.rejected', () => {
    ...
    const action = {type: getUpdateUser.rejected.type, error: {message: errorMessage}};
    const state = reducer(initialState, action);
  ...
  it('должен обрабатывать getUpdateUser.fulfilled', () => {
    const updatedUser = { name: 'Иван Иванов', email: 'ivan.ivanov@example.com' };
    const action = { type: getUpdateUser.fulfilled.type, payload: { user: updatedUser } };
    const state = reducer(initialState, action);
  ...
  it('должен обрабатывать getLogout.rejected', () => {
    ...
    const action = {type: getLogout.rejected.type, error: {message: errorMessage}};
    const state = reducer(initialState, action);
  ...
  it('должен обрабатывать getLogout.fulfilled', () => {
    ...
    const action = { type: getLogout.fulfilled.type, payload: { success: true }, error: null };
  ...
  it('должен обрабатывать getLogout.fulfilled', () => {
    ...
    const action = { type: getLogout.fulfilled.type, payload: { success: false } };
    const state = reducer(initialStateWithUser, action);
    
    expect(state.user).toEqual({ name: 'Иван Иванов', email: 'ivan.ivanov@example.com' });
    expect(state.error).toBe(null);
  });
});
...
describe('getRegisterUser', () => {
  it('должен регистрировать пользователя и сохранять токены', async () => {
    ...
    expect(setCookie).toHaveBeenCalledWith('accessToken', mockResponse.accessToken);
